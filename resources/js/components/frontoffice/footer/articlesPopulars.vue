...
<template>
  ...
  <div v-else class="mb-4 position-relative"  v-for="info in publicationsPopularsData" :key="info.id">
    <div>
      <div class="row g-3">
        <div class="col-4">
          <img class="rounded" style="max-width: 100%; height: 100px" :src="getImage(info.image_cover_url)" :alt="info.title">
        </div>
        <div class="col-8">
          <div><span @click="category(info.category_slug)" style="cursor: pointer" class="badge text-bg-danger mb-2"><i
                class="fas fa-circle me-2 small fw-bold"></i> {{ info.category_name }} </span></div>
          <span @click="article(info.slug)" style="cursor: pointer" class="btn-link text-white "
            v-html="info.title"></span>
          <ul class="nav nav-divider align-items-center small mt-2 text-muted">
            <li class="nav-item position-relative">
              <div class="nav-link">par <span @click="author(info.author_slug)" style="cursor: pointer"
                  class="stretched-link text-reset btn-link"> {{ info.author_name }} </span>
              </div>
            </li>
