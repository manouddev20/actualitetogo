  ...
  <div class="container">
    ...
    <div v-else class="row g-4 p-3 flex-fill">
      ...
      <div
        ...
        <div class="card bg-transparent" v-if="result.cover_media_file && result.cover_media_file.file_url">
          <img class="card-img rounded img-header" :src="getImage(result.cover_media_file.file_url)" :alt="result.title" />
          <div class="card-body px-0 pt-3">
            ...
            <ul class="nav nav-divider align-items-center text-uppercase small mt-2">
              <li class="nav-item">
                <span @click="author(result.author.slug)" class="text-reset btn-link" style="cursor: pointer">{{ result.author.name }}</span>
              </li>
        ...
        <div v-else>
          <div class="card mb-4">
            <div class="card-body border rounded-3">
                ...
                <ul class="nav nav-divider align-items-center text-uppercase small mt-2">
                    <li class="nav-item">
                        <span @click="author(result.author.slug)" style="cursor: pointer" class="text-reset btn-link">{{ result.author.name }}</span>
                    </li>
                    <li class="nav-item">{{ moment(result.date_publish).format("DD/MM/YYYY") }}</li>
                </ul>
            </div>
